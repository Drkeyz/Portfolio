...
function ProjectCard({ p }) {
  return (
    <Link to={p.route} className="group flex flex-col gap-3 cursor-pointer">
      ...
      <div className="flex items-start justify-between">
        <h3 className="text-sm md:text-base font-medium text-[#0a0c0d] pr-2">{p.title}</h3>
        <div className="text-right text-xs md:text-sm text-[#4a5565] leading-relaxed shrink-0">
          <div>{p.year}</div>
        </div>
      </div>
    </Link>
...
export default function MoreProjectsPage() {
  ...
  return (
    <div className="min-h-screen bg-white" style={{ color: '#0a0c0d' }}>
      ...
      <main className="mx-auto max-w-[1440px] px-5 md:px-8 pt-24 pb-16" id="projects">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-8">
          <h1
            className="text-[clamp(28px,6vw,48px)] font-medium"
            style={{ fontFamily: '"Instrument Serif", Georgia, serif' }}
          >
            More projects
          </h1>
          <p className="text-sm text-[#4a5565] md:w-1/3 leading-relaxed">
            Some of these projects aren't part of the main portfolio; some are projects I played around with and decided to include.
          </p>
        </div>
