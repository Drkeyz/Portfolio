...
/* ─── Section header: label + centred heading ───────────────── */
/*
  Desktop: [label pill] ───── [centred h2]
  Mobile:  label above heading, both full-width
*/
...
function StatTile({ value, label }) {
  return (
    <div>
      <p className="text-xl md:text-2xl font-semibold text-[#8B5CF6]">{value}</p>
      <p className="text-sm md:text-base mt-1 md:mt-2" style={{ color: SLATE_LIGHT }}>{label}</p>
    </div>
  )
}
...
function InfoCard({ title, label, children }) {
  return (
    <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-6 md:p-8 shadow-sm hover:border-[#8B5CF6] transition-colors">
      <p className="text-xs uppercase tracking-[0.18em] mb-3" style={{ color: SLATE_LIGHT }}>{label}</p>
...
function FeatureCard({ title, children, className, icon, imageUrl, imageBackground }) {
  return (
    <div className={`bg-white p-6 md:p-8 transition-colors ${className}`}>
      {icon && (
        <div className={`mb-4 w-[38px] h-[38px] rounded-[0.625rem] flex flex-col justify-center items-center ${imageBackground}`}>
          <img src={imageUrl} className="h-4 w-4" alt="" />
        </div>
      )}
      <h3 className="text-xl md:text-2xl mb-3" style={{ color: INK, fontFamily: SERIF }}>{title}</h3>
      <p className="text-sm leading-relaxed" style={{ color: SLATE }}>{children}</p>
    </div>
  )
}
...
function ProcessStep({ number, title, children, active }) {
  return (
    <div className={`group border border-[#eeeeee] bg-white p-5 md:p-[18px] shadow-sm hover:border-b-[#8B5CF6] hover:border-b-2 transition-colors`}>
      <p className="text-3xl md:text-4xl text-[#eeeeee] mb-4 group-hover:text-[#8B5CF6]/30">{number}</p>
      <h4 className="text-xl md:text-2xl mb-3" style={{ color: INK, fontFamily: SERIF }}>{title}</h4>
      <p className="text-sm leading-relaxed" style={{ color: SLATE }}>{children}</p>
    </div>
  )
}
...
/* ─── Pill card ─────────────────────────────────────────────── */
const TAG_COLORS = {
  'For Investors':   '#ECFDF5',
  'For Businesses':  '#EFF6FF',
  'For Researchers': '#ECFEFF',
  'For Compliance':  '#FEF3C7',
}

function PillCard({ tag, title, children }) {
  return (
    <div className="border border-[#eeeeee] bg-white p-6 md:p-8 transition-colors">
      <span
        className="inline-flex rounded-full px-3 py-1 text-[11px] uppercase tracking-[0.22em] font-semibold mb-4"
        style={{ background: TAG_COLORS[tag] || '#f3f4f6', color: INK }}
      >
        {tag}
      </span>
      <h4 className="text-lg md:text-xl mb-3" style={{ color: INK, fontFamily: SERIF }}>{title}</h4>
...
export default function ArgusPage() {
  ...
  return (
    <div className="min-h-screen bg-white" style={{ color: INK }}>
      {/* Uses the shared Navbar component */}
      <Navbar />

      {/* ── HERO ──────────────────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pt-24 md:pt-28 pb-12 md:pb-20">
        ...
        {/* Meta row */}
        <div className="flex flex-wrap items-center gap-4 md:gap-8 text-xs uppercase tracking-[0.16em] mb-6" style={{ color: SLATE_LIGHT }}>
          <span>PROJECT / ARGUS</span>
          <span>SCOPE / LANDING PAGE</span>
        </div>
      ...
      {/* ── [01] OVERVIEW ─────────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[01] Overview">
          One dashboard. <span className="italic" style={{ color: SLATE }}>Total access.</span>
        </SectionHeader>

        <div className="mt-10 md:mt-16 grid gap-5 md:gap-6 sm:grid-cols-2">
          <InfoCard label="The product" title="Argus">
            Argus is an AI-powered blockchain analytics platform designed to give crypto professionals unparalleled visibility across the entire blockchain ecosystem — from wallet behaviour to smart contract risks.
          </InfoCard>
          <InfoCard label="My role" title="Product Designer">
      ...
      {/* ── [02] DESIGN ───────────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[02] Design">
          <span className="italic" style={{ color: SLATE }}>The</span> full{' '}
          <span className="italic" style={{ color: SLATE }}>design.</span>
        </SectionHeader>

        <div className="grid gap-5 md:gap-8 lg:grid-cols-2 mt-10 md:mt-16">
          <div className="overflow-hidden rounded-[2rem] border border-[#E5E7EB] bg-[#0f172a]">
            <img src="/Top half.png" alt="Argus landing page preview"
              className="w-full h-full object-cover" style={{ minHeight: '240px' }}
              onError={e => { e.currentTarget.style.display = 'none' }} />
          </div>
          <div className="overflow-hidden rounded-[2rem] border border-[#E5E7EB] bg-[#0f172a]">
            <img src="/Bottom half.png" alt="Argus design details preview"
              className="w-full h-full object-cover" style={{ minHeight: '240px' }}
              onError={e => { e.currentTarget.style.display = 'none' }} />
          </div>
        </div>
      ...
      {/* ── [03] CHALLENGE ────────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[03] Challenge">
          <span className="italic" style={{ color: SLATE }}>The</span> complexity{' '}
          <span className="italic" style={{ color: SLATE }}>was the</span> enemy.
        </SectionHeader>

        <div className="grid gap-4 md:gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-10 md:mt-16">
          <FeatureCard title="Data overload without context" className="border rounded-2xl">
            Blockchain data is vast and raw. Users needed not just access to data, but a way to understand it at a glance — patterns, anomalies, and signals surfaced without manual effort.
          </FeatureCard>
          <FeatureCard title="Four very different user types" className="border rounded-2xl">
            Investors, businesses, researchers, and compliance teams all had radically different needs, workflows, and mental models. A single interface had to flex intelligently across all four.
          </FeatureCard>
          <FeatureCard title="Making AI feel trustworthy" className="border rounded-2xl">
            AI-generated insights in a high-stakes financial context had to feel transparent, reliable, and explainable. The design needed to build trust at every touchpoint.
          </FeatureCard>
        </div>
      </section>
      ...
      {/* ── [04] PROCESS ──────────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[04] Process">
          How we <span className="italic" style={{ color: SLATE }}>got</span> here.
        </SectionHeader>

        {/* 1-col → 2-col → 5-col */}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 md:gap-0 mt-10 md:mt-16">
          <ProcessStep number="01" title="Discovery" active>
            Interviewed crypto investors, compliance leads, and on-chain researchers to map workflows, pain points, and mental models around blockchain data.
          </ProcessStep>
          <ProcessStep number="02" title="Architecture">
      ...
      {/* ── [05] KEY FEATURES ─────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[05] Key Features">
          What Argus can do
        </SectionHeader>

        {/* 1-col → 2-col → 3-col, shared border grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mt-10 md:mt-16 border-t border-l border-[#eeeeee]">
          {[
            { title: 'AI-Enhanced Wallet Labelling', imageUrl: '/pulse.svg',   imageBackground: 'bg-[#11f611]/10', body: 'ML models automatically identify and categorize wallet types — exchanges, whales, DeFi protocols — removing hours of manual tagging.' },
            { title: 'Predictive Analytics',         imageUrl: '/diamond.svg', imageBackground: 'bg-[#3c0366]/10', body: 'Time-series forecasting tracks blockchain metrics and surfaces early signals, helping users act ahead of the market.' },
            { title: 'Anomaly Detection',            imageUrl: '/warning.svg', imageBackground: 'bg-[#d91316]/10', body: 'Continuous monitoring flags unusual on-chain activity — sudden wallet movements, contract exploits, or suspicious clustering in real time.' },
            { title: 'Smart Contract Analysis',      imageUrl: '/cube.svg',    imageBackground: 'bg-[#10afee]/10', body: 'Deep inspection of contract behaviour, risk scoring, and interaction patterns — confidence before engaging with any protocol.' },
            { title: 'Personalized Insights',        imageUrl: '/moon.svg',    imageBackground: 'bg-[#b4b707]/10', body: "AI recommendations tailored to each user's portfolio, watchlist, and behaviour — the platform gets smarter the more you use it." },
            { title: 'Natural Language Interface',   imageUrl: '/stars.svg',   imageBackground: 'bg-[#510424]/10', body: 'Query the entire blockchain dataset in plain English. No SQL, no technical overhead — ask a question, get an answer with supporting data instantly.' },
          ].map(({ title, imageUrl, imageBackground, body }) => (
            <FeatureCard
              key={title}
              title={title}
              icon
      ...
      {/* ── [06] SOLUTIONS ────────────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[06] Solutions">
          Designed <span className="italic" style={{ color: SLATE }}>for every</span> professional.
        </SectionHeader>

        {/* 1-col → 2-col, shared border grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 mt-10 md:mt-16 border-t border-l border-[#eeeeee]">
          {[
            {
              tag: 'For Investors', title: 'Data-driven investment decisions',
              body: 'The investor view surfaces portfolio risk, whale movement alerts, and market-timing signals in a single scannable dashboard.',
              bullets: ['Real-time whale wallet tracking and alerts', 'AI-scored risk assessment per asset', 'Portfolio exposure across 27 chains'],
            },
            {
              tag: 'For Businesses', title: 'Optimize with blockchain intelligence',
              body: 'Businesses get automated monitoring, competitor on-chain analysis, and API access to embed Argus intelligence into their own products.',
              bullets: ['Custom monitoring dashboards per use case', 'Flexible API and SDK integrations', 'Competitor wallet and protocol tracking'],
            },
            {
              tag: 'For Researchers', title: 'Deeper on-chain insights',
              body: 'Researchers get raw data access, interactive graph exploration, NLP querying, and exportable normalized datasets for serious investigation.',
              bullets: ['Interactive blockchain graph visualizations', 'NLP querying across full data history', 'Exportable normalized datasets'],
            },
            {
              tag: 'For Compliance', title: 'Stay compliant with ease',
              body: 'Automated AML screening, regulatory reporting, smart contract auditing, and a complete audit trail with human-readable explanations built in.',
              bullets: ['Automated AML and sanctions screening', 'Regulatory report generation', 'Smart contract risk scoring and audit logs'],
            },
          ].map(({ tag, title, body, bullets }) => (
            <PillCard key={tag} tag={tag} title={title}>
              <p>{body}</p>
      ...
      {/* ── [07] DESIGN DECISIONS ─────────────────────────────── */}
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 pb-12 md:pb-20">
        <SectionHeader tag="[07] Design Decisions">
          <span className="italic" style={{ color: SLATE }}>The</span> thinking{' '}
          <span className="italic" style={{ color: SLATE }}>behind the</span> work.
        </SectionHeader>
        ...
        <div className="flex flex-col xl:flex-row gap-8 mt-10 md:mt-16">
          ...
          <div className="bg-white xl:w-[581px]">
            {[
              { title: 'Dark-first visual language',                  body: 'Financial and data platforms live in dark environments — trading terminals, monitoring screens, late-night research sessions. Dark-first reduced eye strain across long sessions and made data visualizations dramatically more legible.' },
              { title: 'Progressive disclosure over feature dumping', body: 'With six distinct feature areas and four user types, the risk of cognitive overload was real. Information architecture was structured around progressive disclosure — surface the signal, hide the noise.' },
              { title: 'Trust through transparency in AI output',     body: 'Every AI-generated insight was designed to show its reasoning — confidence scores, supporting data points, and clear labelling of what was predicted vs observed.' },
              { title: 'Visual hierarchy through typography scale',   body: 'Rather than leaning on colour to create hierarchy, we used aggressive typographic scale — large display headings, mono for metadata, small regular weight for body.' },
            ].map(({ title, body }) => (
              <div key={title} className="border border-[#8B5CF6] border-l-0 border-r-0 border-t-0 first:border-t px-6 py-[14px]">
                <h3 className="text-xl md:text-2xl mb-3" style={{ color: INK, fontFamily: SERIF }}>{title}</h3>
          ...
          {/* Principles */}
          <div className="flex flex-col gap-3 xl:w-[431px] xl:my-auto">
            {[
              'P1 Signal over noise — always surface the most important insight first',
              'P2 Trust is designed, not assumed — show the work behind every AI claim',
              'P3 Complexity lives in the data, not the interface',
              ...
              <div key={item} className="bg-[#eeeeee]/60 p-5 md:p-6">
                <p className="text-sm leading-relaxed" style={{ color: '#4A5565' }}>{item}</p>
              </div>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-[1440px] px-5 md:px-8 py-12 md:py-20">
        <SectionHeader tag="[08] Learnings">
          <span className="italic" style={{ color: SLATE }}>What I'd</span> take{' '}
          <span className="italic" style={{ color: SLATE }}>into the</span> next one.
        </SectionHeader>

        {/* 1-col → 3-col, shared border grid */}
        <div className="mt-10 md:mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 border-t border-l border-[#eeeeee]">
          {[
            { icon: '●', title: 'Domain literacy changes everything', body: 'Spending time learning the actual mechanics of blockchain, not just the user needs, made the design decisions sharper. The best insights came from understanding the data at a technical level, not just an abstract one.' },
            { icon: '◆', title: 'Persona-driven design has limits',   body: "Real users don't stay in their lane. An investor also does compliance checks. A researcher sometimes needs investor-style alerts. Designing for flexibility within persona frameworks was more valuable than strict isolation." },
            { icon: '★', title: 'AI UI is an emerging discipline',    body: "There's no established playbook for how to present AI-generated insights in high-stakes contexts. Experimentation, user testing, and humility about what we didn't know led to better outcomes than relying on convention." },
          ].map(({ icon, title, body }) => (
            <div key={title} className="border-b border-r border-[#eeeeee] bg-white p-6 md:p-8 transition-colors">
              <div className="mb-5 md:mb-6 inline-flex h-12 w-12 items-center justify-center rounded-full border border-[#0a0c0d] text-[#0a0c0d]">
                <span className="text-xl">{icon}</span>
              </div>
              <h3 className="text-xl md:text-2xl mb-4" style={{ color: INK, fontFamily: SERIF }}>{title}</h3>
              <p className="text-sm leading-relaxed" style={{ color: SLATE }}>{body}</p>
            </div>
          ))}
        </div>
